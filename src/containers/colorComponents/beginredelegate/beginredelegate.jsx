...
export default function Redelegate() {
  ...
  const [fromCopied, setFromCopied] = React.useState(false);
  ...
  function redelegate(address, password, signMessage) {
    let wallet;
    let signature;
  ...
  function setFrom() {
    ...
    setToCopied(false);
    setFromCopied(true);
    setTimeout(() => setFromCopied(false), 2000);
  }
  function setTo() {
    setFromCopied(false);
    setCopied(false);
                ...
                >
                  ...
                  >
                    <p>
                      &nbsp;Redelegated&nbsp;
                      <b>
                        {latestSignReq.msgs[0].value.amount.amount / 1000000}
                        &nbsp;
                      </b>
                      <span>&nbsp;CLR</span>
                    </p>
                  ...
                  <div className="tx__content__information">
                    ...
                    <a className="">
                      <CopyToClipboard
                        text={latestSignReq.msgs[0].value.validator_src_address}
                        onCopy={() => setFrom()}
                      >
                        <span>
                          {latestSignReq.msgs[0].value.validator_src_address.substr(
                            0,
                            6
                          ) +
                            '...' +
                            latestSignReq.msgs[0].value.validator_src_address.substr(
                              latestSignReq.msgs[0].value.validator_src_address
                                .length - 4,
                              latestSignReq.msgs[0].value.validator_src_address
                                .length - 1
                            )}
                    ...
                    <a className="">
                      <CopyToClipboard
                        text={latestSignReq.msgs[0].value.validator_dst_address}
                        onCopy={() => setTo()}
                      >
                        <span>
                          {latestSignReq.msgs[0].value.validator_dst_address.substr(
                            0,
                            6
                          ) +
                            '...' +
                            latestSignReq.msgs[0].value.validator_dst_address.substr(
                              latestSignReq.msgs[0].value.validator_dst_address
                                .length - 4,
                              latestSignReq.msgs[0].value.validator_dst_address
                                .length - 1
                            )}
